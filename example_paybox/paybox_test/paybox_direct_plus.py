...
class PayboxDirectPlusTransaction:
    """A Paybox Direct Plus transaction, from your server to Paybox server
        Attributes:
    ...
    def __init__(self, production=False, dateq=None, operation_type=None, numquestion=None, montant=None,
                 ...
                 sha_1=None, errorcodetest=None, id_merchant=None, id_session=None, url_http_direct=None, secure_3d=False):

        self.production = production
        self.RETRY_CODES = ["00001", "00097", "00098"]
        self.session_id = id_session
        self.merchant_id = id_merchant
        self.secure_3d = secure_3d
        self.id3d = None
        self.subscriber_registered = False
        ...
        self.MANDATORY = {
            ...
            "DEVISE": devise,  # Currency code of the transaction
        ...
        self.OPERATION_TYPES = {
            "00051": "Authorization only on a subscriber",
            "00052": "Debit on a subscriber",
            "00053": "Authorization + Capture on a subscriber",
            "00054": "Credit on a subscriber",
            "00055": "Cancel subscriber transaction",
            "00056": "Register new subscriber",
            "00057": "Update an existing subscriber",
            "00058": "Delete a subscriber",
            "00061": "Authorization only",
        }

        self.REGISTERED_USER_OPERATIONS = [
            "00051", "00052", "00054", "00055", "00057", "00058"
        ]
    ...
    def remote_mpi_url(self):
        """
        This method returns a working remote_mpi_url to be used
        for 3D-Secure transaction authentication
        :return: str
        """
        if self.production:
            ...
            request = requests.get(remote_mpi_url)
            if request.status_code == 200:
    ...
    def remote_mpi_authenticate(self, session_id):
        ...
        """
        card_verification_string = "IdMerchant={0},IdSession={1},Amount={2},Currency={3},CCNumber={4},CCExpDate={5},CVVCode={6},URLHttpDirect={7}".format(
            settings.PAYBOX_IDENTIFIANT, session_id, self.MANDATORY['MONTANT'], self.MANDATORY['DEVISE'],
            self.MANDATORY['PORTEUR'], self.MANDATORY['DATEVAL'], self.ACCESSORY['CVV'],
            self.card_verification_api_redirect_url)
    ...
    def post_to_paybox(self, numquestion, operation_type=None):
        ...
        if self.MANDATORY['TYPE'] in self.REGISTERED_USER_OPERATIONS:
            if not self.subscriber_registered:
                raise InvalidPaymentSolutionException(
                    message="You need to be a registered subscriber to make this call.")
        if self.MANDATORY['TYPE'] in self.DELAY_OPERATIONS:
            sleep(1)
        ...
        if self.MANDATORY['TYPE'] == "00056":
            self.ACCESSORY['NUMAPPEL'] = response['NUMAPPEL']
            self.ACCESSORY['NUMTRANS'] = response['NUMTRANS']
            self.subscriber_registered = True
        return response

    def construct_html_form(self):
        """
        Returns an html form ready to be POSTed to Paybox Direct Plus(string)
        :return: str <form>
        """
