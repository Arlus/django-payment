...
def add_transfer_data(charge_payload, destination, percent):
    ...
    logger.debug('paybox_connect_maybe_add_transfer_data',
...
def maybe_add_transfer_data(charge_payload):
    if hasattr(settings, 'PAYBOX_CONNECT'):
        _connect_settings = settings.PAYBOX_CONNECT
        ...
        try:
            ...
            raise Exception("PAYBOX_TRANSFER_PERCENT should be an int")
        if _transfer_percent < 0 or _transfer_percent > 100:
            raise Exception("PAYBOX_TRANSFER_PERCENT should be between 0 and 100")
